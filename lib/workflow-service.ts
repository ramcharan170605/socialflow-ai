...
import { buildMediaUrls } from '@/lib/media/storage';
...
function mediaContextLine(mediaAssets?: MediaAsset[]): string {
  ...
  const list = mediaAssets
    .map((m, i) => `[Image ${i + 1}: ${m.filename} — ${m.url}]`)
    .join('\n');
...
export async function triggerUserWorkflow(
  ...
  const mediaAssets = (input.mediaAssets ?? []).map((asset) => ({
    ...asset,
    ...buildMediaUrls(input.userId, asset.id),
  }));
